...
#define _DIST_MED 200.0   // median distance to be measures (unit: mm)
#define _DIST_MAX 300.0   // maximum distance to be measured (unit: mm)
#define BRIGHT_MM 2.56    // analog light per 1 mm

#define TIMEOUT ((INTERVAL / 2) * 1000.0) // maximum echo waiting time (unit: usec)
#define SCALE (0.001 * 0.5 * SND_VEL) // coefficent to convert duration to distance

unsigned long last_sampling_time;   // unit: msec
float last_distance = 300.0;                // unit: mm
...
void loop() {
  ...
  if (distance > _DIST_MAX || distance < _DIST_MIN){
    ...
    analogWrite(PIN_LED, abs(_DIST_MED - distance)* BRIGHT_MM);
  } else {
    analogWrite(PIN_LED, abs(_DIST_MED - distance)* BRIGHT_MM);
  }
...
//  Serial.print("Bright: ");    Serial.print(abs(_DIST_MED - distance)* BRIGHT_MM);
//  Serial.println("");
